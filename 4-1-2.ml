module Lexer = struct
module P = Printf
type token = CID of string | VID of string | NUM of string | TO | IS | QUIT 
             | OPEN | EOF | ONE of char
...
else if (c = ':') then (read();let d = lookahead() in if (d = '-') then (read(); TO)
                                                     else native_token ())
else ONE (read ())
and gettoken () =
try
